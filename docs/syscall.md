...
| Syscall | 번호 | 시그니처 | 설명 |
|---------|------|----------|------|
| `sys_epoll_create1` | 20 | `epoll_create1(flags) -> epfd` | epoll 인스턴스 생성 (`EPOLL_CLOEXEC` 허용, 기타 flags는 `EINVAL`) |
| `sys_epoll_ctl` | 21 | `epoll_ctl(epfd, op, fd, event)` | ADD/MOD/DEL + watch data/events 갱신 + one-shot 재장전 |
| `sys_epoll_pwait` | 22 | `epoll_pwait(epfd, events, maxevents, timeout, sigmask, sigsetsize)` | ready scan + timeout + `sigmask` 적용 + `EPOLLET/EPOLLONESHOT` |
...
| `sys_pselect6` | 72 | `pselect6(nfds, readfds, writefds, exceptfds, timeout, sigmask)` | `fd_set` ready scan + timeout + Linux `sigmask` arg 적용 |
| `sys_ppoll` | 73 | `ppoll(fds, nfds, timeout, sigmask, sigsetsize)` | `pollfd` ready scan + timeout + `sigmask/sigsetsize` 적용 |
...
- `pselect6`/`ppoll`/`epoll_pwait`는 호출 중 임시 `sigmask` 적용 후 원래 마스크를 복원합니다.
- `epoll`은 `EPOLLET`/`EPOLLONESHOT` + fd wait queue wakeup(`read/write/close`) 경로를 지원합니다.
- `epoll_event` 유저 ABI는 Linux와 동일한 packed 12-byte 레이아웃을 사용합니다.
