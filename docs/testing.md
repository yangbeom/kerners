...
| `scripts/build_user_dynamic_c_bins.sh [ARCH] [OUT_DIR]` | C 계열(`clang` + `rust-lld`) 최소 동적 ELF(`hello_dyn`, `busybox_dyn`, `ld-kerners-*.so`) 생성 |
...
| `scripts/run_user_tests.sh [ARCH] [TIMEOUT]` | 유저 테스트 오케스트레이션 (`verify_phase15_3_cdyn.sh` + `verify_phase15_3_busybox_dyn.sh`) |
...
| `make test-user` | 유저 영역 동적 hello + 동적 busybox(init) 스모크를 aarch64/riscv64 모두 실행 |
